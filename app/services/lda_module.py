...
#             header='',  # Change to True if topic id should be present as the header
    ...
    def get_topic_share(self, topic_id, doc_ids, serialize=False):
        ...
        topic_share = self.normalized_topics_by_documents.reindex(doc_ids)[topic_id].to_dict()
    ...
    def get_docs_by_topic_composition(self, topic_percentage, topn=10, return_data='id', closest_to_minimum=False, return_similarity=False, duplicate_threshold=0.01, show_duplicates=True, serialize=False):
        ...
        topic_percentage = topic_percentage[topic_percentage > 0]  # Just in case we set the default values to zero
        ...
        candidate_docs = candidate_docs.sort_values('score', ascending=closest_to_minimum)  # Set to ascending=False if we want to show documents with higher topic composition than the given minimum. Use ascending=False if we want to get the documents with topic composition closest to the given minimum.
