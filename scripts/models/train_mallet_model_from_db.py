...
from wb_nlp.types.models import ModelRunInfo, MalletModelConfig
...
def main(model_config_id: str, cleaning_config_id: str, log_level: int):
    ...
    with contexttimer.Timer() as timer:
        ...
        MalletModelConfig(**model_config)

        model_name = model_config["meta"]["model_name"]

        assert model_name == "mallet"
        ...
        model_file_name = model_dir / f'model_{model_run_info_id}.mallet.bz2'
        ...
        if not model_file_name.exists():
            ...
            mallet_params = model_config["mallet_config"]
            mallet_params['id2word'] = dict(g_dict.id2token)
            mallet_params.pop('mallet_config_id')

            root_dir = Path(dir_manager.get_path_from_root())

            mallet_path = root_dir / mallet_params["mallet_path"]
            prefix = root_dir / mallet_params["prefix"]

            assert mallet_path.exists()

            if not prefix.parent.exists():
                prefix.parent.mkdir(parents=True)

            mallet_params["mallet_path"] = str(mallet_path)
            mallet_params["prefix"] = str(prefix)

            mallet = LdaMallet(corpus=corpus, **mallet_params)
...
if __name__ == '__main__':
    # python -u ./scripts/models/train_mallet_model_from_db.py --model-config-id 5e26b5090bdd91d7aee4e5e89753a33b --cleaning-config-id 23f78350192d924e4a8f75278aca0e1c -vv |& tee ./logs/train_mallet_model_from_db.py.log
