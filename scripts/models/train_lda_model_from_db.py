...
def main(model_config_id: str, cleaning_config_id: str, log_level: int, load_dictionary: bool, load_dump: bool):
    '''
    ...
    with contexttimer.Timer() as timer:
        ...
        models_config_collection = mongodb.get_models_config_collection()
        model_config = models_config_collection.find_one(
            {"_id": model_config_id})
        ...
        file_generator = MultiDirGenerator(
            ...
            split=True,
            min_tokens=model_config['params']['min_tokens'],
            logger=_logger
        )

        _logger.info('Training dictionary...')
        dictionary_params = model_config['params']['dictionary']
        # dictionary_hash = generate_model_hash(dictionary_params)

        dictionary_file = cleaned_docs_dir / \
            f"dictionary-{cleaned_corpus_id}.gensim.dict"

        corpus_path = cleaned_docs_dir / f"bow_corpus-{cleaned_corpus_id}.mm"
