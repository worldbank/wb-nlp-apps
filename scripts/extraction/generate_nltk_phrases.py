...
import joblib
import wb_nlp
from wb_nlp.extraction.phrase import get_nltk_phrases
...
def joblib_extract_nltk_phrases(
    ...
    with open(input_file, "rb") as in_file:
        ...
        phrases = get_nltk_phrases(text)
        phrases = dict(Counter(phrases).most_common())
        result = dict(lib='NLTK', tokens=[], phrases=phrases)
...
def main(input_dir: Path, output_dir: Path, log_level: int, n_workers: int = None):
    ...
    with joblib.parallel_backend('dask'):
        res = Parallel(verbose=10)(
            delayed(joblib_extract_nltk_phrases)(
...
if __name__ == '__main__':
    # python -u ./scripts/extraction/generate_nltk_phrases.py --input-dir ./data/raw/sample_data/TXT_SAMPLE --output-dir ./data/preprocessed/sample_data/nltk_phrases -v --n-workers 6 |& tee generate_nltk_phrases.py.log
    main()
