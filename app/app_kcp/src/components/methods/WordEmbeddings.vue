<template>
  <div class="text-justify">
    ...
    <div>
      ...
      <p>
        Early techniques to convert text to vector includes bag-of-words—a
        strategy that builds a vector of values corresponding to the count of
        each word found in a document. Example we have a text
        <i>“the dog and the cat.”</i> This could be converted into a vector with
        values [1, 1, 1, 2]. To come up with this vector, the unique words in
        the text are extracted and sorted alphabetically—and, cat, dog, the.
        Then, the vector is filled with the count of each word. The count for
        the word is placed based on the order that the words are sorted—and=1 is
        placed in the first position, cat=1 in the second, dog=1 in the third,
        and the=2 is placed in the fourth position.
      </p>
      <p>
        ...
        into numeric values, one of the most common is TF-IDF. A curious
        observer will notice that the bag-of-words method has plenty of short
        comings, one of which is that it is purely a lexical representation.
        This means that if our application is designed to find relevant
        documents given a set of words, then texts without the keywords will not
        be considered despite being relevant. Example, a text without the word
        <i>“malnutrition”</i> but talks about <i>“stunting”</i> will be ignored
        despite it being likely to be relevant. This issue is one of the main
        problems that word embeddings attempt to solve.
      </p>
      <p>
      ...
      <p>
        ...
        in Gensim. The word2vec is parametrized by some parameters but the main
        parameters that we experimented with are the window, size or dimension,
        min_count, number of negative samples for the skip-gram implementation,
      ...
      <p>
        ...
        <i>“blue”</i> and our window is set to 2, then the context words
        (without stop words removal) will include <i>“sky”</i> and
        <i>“is”</i> (left context) and <i>“now”</i> and <i>“but”</i> (right
        context). The model will try to learn a vector for blue that is
        generally close to these words. Different contexts may appear over
        different occurrences of the word <i>“blue”</i> in the whole corpus.
      </p>
      <p>
